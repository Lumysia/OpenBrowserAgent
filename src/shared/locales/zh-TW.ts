import type { LocaleMessages } from "./en-US";

export const zhTW: LocaleMessages = {
  app: { tagline: "瀏覽器中的 AI 智慧體" },
  common: {
    add: "新增",
    cancel: "取消",
    delete: "刪除",
    save: "儲存",
    saved: "已儲存",
    settings: "設定",
    copy: "複製",
    copied: "已複製",
    edit: "編輯",
    help: "說明",
    language: "語言",
    enabled: "已啟用",
    disabled: "未啟用",
    noModelsFound: "找不到模型。",
    confirm: "確認",
  },
  words: { newChat: "新對話", agent: "瀏覽", ask: "詢問" },
  sidepanel: {
    connectProviderTitle: "初始化 OpenBrowserAgent",
    connectProviderDescription:
      "選擇介面語言，可選設定同步，然後新增提供者與模型即可開始。",
    addProvider: "新增提供者",
    cloudSync: "從雲端同步",
    cloudSyncTest: "測試雲端同步",
    cloudSyncTesting: "正在測試雲端同步...",
    cloudSyncPulling: "正在檢查雲端同步...",
    cloudSyncDescription:
      "可從雲端拉取其他裝置的設定，也可以把目前裝置的新設定儲存到雲端。",
    cloudSyncReady: "已找到同步的提供者設定，可以從雲端開始。",
    cloudSyncSuccess: "已找到同步的提供者設定，正在開啟對話...",
    cloudSyncEmpty:
      "雲端連線成功，但找不到既有設定。確認後會把新的設定儲存到雲端。",
    cloudSyncError: "雲端同步測試失敗。請檢查後端，或從本機開始。",
    cloudSyncStoreNew: "將新設定存放到雲端",
    confirmCloudSync: "確認同步",
    cloudSyncSynced: "雲端同步已準備好。接下來新增提供者設定。",
    cloudSyncSyncedButton: "已同步",
    startLocally: "從本機開始",
    startFromCloud: "從雲端開始",
    localSetupHint:
      "設定頁已開啟。請在那裡新增提供者和模型，然後回到這裡繼續。",
    bootstrapStepSyncTitle: "同步後端（可選）",
    syncConfigCodeTab: "貼上配置碼",
    syncManualTab: "手動輸入",
    syncConfigCode: "同步配置碼",
    syncConfigCodeDescription: "貼上從另一台裝置複製的配置碼。",
    syncConfigCodeApply: "套用配置碼",
    syncConfigCodeApplied: "同步後端配置已套用，可以繼續檢查雲端同步。",
    syncConfigCodeInvalid: "配置碼無效，請檢查後再試。",
    bootstrapStepProviderTitle: "新增提供者和模型",
    bootstrapStepProviderDescription:
      "開啟設定頁，新增提供者、新增模型並測試是否可用。",
    bootstrapStepTaskTitle: "傳送第一個任務",
    bootstrapStepTaskDescription:
      "模型測試通過後回到這裡，讓 OpenBrowserAgent 瀏覽、摘要或執行任務。",
    menu: "選單",
    chatHistory: "對話記錄",
    backToChat: "返回對話",
    exportChatOpenAi: "匯出為 OpenAI JSON",
    importChatOpenAi: "匯入 OpenAI JSON 對話",
    clearAllChats: "清空所有對話",
    noChatsYet: "尚無對話",
    removeChat: "移除對話",
    subAgentChat: "子智慧體聊天",
    subAgentBadge: "子",
    backToParentChat: "返回父聊天",
    openSubAgentChat: "開啟子智慧體聊天",
    subAgentRunning: "執行中",
    subAgentCompleted: "已完成",
    subAgentMissing: "找不到",
    localExecutionBridgeStates: {
      running: "執行中",
      done: "已完成",
      error: "錯誤",
      missing: "找不到",
      canceled: "已取消",
    },
    questionWaiting: "等待你的回答",
    questionSubmit: "提交回答",
    questionCustomPlaceholder: "輸入自訂回答……",
    questionNoAnswer: "未回答",
    forkTitleSuffix: "分叉",
    messageCount: "{count} 則訊息",
    relativeMinutesAgo: "{count} 分鐘前",
    relativeHoursAgo: "{count} 小時前",
    relativeDaysAgo: "{count} 天前",
    whatDoYouWant: "你想做什麼？",
    emptyDescription: "自動化任務、搜尋並操作你的瀏覽器。",
    aiWorking: "AI 正在工作",
    aiWorkingDescription: "正在閱讀、操作並準備回覆。",
    typingPhrases: [
      "琢磨中...",
      "偵查中...",
      "拆解中...",
      "定位中...",
      "捋線中...",
      "提煉中...",
      "變魔法中...",
      "除錯中...",
      "打磨中...",
      "對上號中...",
    ],
    runInfo: {
      title: "執行資訊",
      currentTurn: "目前回合",
      wholeChat: "整個對話",
      ttft: "首字延遲",
      tps: "每秒 Token",
      output: "輸出方式",
      streaming: "串流輸出",
      buffered: "非串流輸出",
      mixed: "混合",
      input: "輸入",
      outputTokens: "輸出",
      total: "總計",
      cached: "快取",
      cacheWrite: "寫入快取",
      reasoning: "推理",
      estimated: "約",
      tokenUnit: "tok.",
      estimatedTokenCount: "約 {count} tok.",
      promptBreakdown: "提示詞組成",
      contextSaved: "已節省上下文",
      contextPrunedMessages: "已裁剪訊息",
      contextToolResults: "已裁剪工具結果",
      prebuiltPrompt: "預製提示詞",
      userPrompt: "使用者提示詞",
      conversationPrompt: "對話歷史",
      tabPrompt: "分頁 / 頁面",
      selectedElementPrompt: "已選元素",
      skillPrompt: "技能",
      attachmentPrompt: "附件",
      toolCallPrompt: "工具呼叫",
      sourcePrompt: "來源",
      otherPrompt: "其它上下文",
      characters: "字元",
      unavailable: "不可用",
    },
    operatingBrowser: "正在操作網頁",
    preparingFinalReply: "正在準備最終回覆",
    preparingFinalReplyDescription: "瀏覽器操作已完成，AI 正在整理結果。",
    willBeSentToAi: "將傳送給 AI",
    willBeSentAsPageContext: "將作為頁面上下文傳送",
    currentPage: "目前頁面",
    elementSelected: "已選擇元素",
    addNewTab: "新增分頁",
    addCurrentTab: "新增目前分頁",
    attachFiles: "上傳檔案",
    attachFilesHint: "圖片或本機檔案",
    removeAttachment: "移除附件",
    removeTab: "移除分頁",
    attachmentTooLarge: "部分檔案過大，無法加入：{names}",
    attachmentsUnsupportedRetry:
      "目前模型拒絕了圖片/檔案負載，已自動改用文字和檔案資訊重試。",
    attachmentOnlyMessage: "請查看附件內容。",
    fileMetadataOnly: "僅傳送檔案資訊",
    replaceUnavailableAttachments: "替換不可用檔案",
    replaceUnavailableAttachmentsDescription:
      "選擇替換檔案，或不帶這些檔案重發。",
    resendWithoutMissingFiles: "不帶缺失檔案重發",
    resendMessage: "重發",
    forkChat: "分叉對話",
    showMoreSources: "展開剩餘 {count} 個來源",
    showFewerSources: "收起來源",
    unavailableAttachments: "{count} 個不可用",
    allOpenTabs: "所有開啟的分頁",
    selectElement: "選擇元素",
    selectModel: "選擇模型",
    reasoningEffort: "推理強度",
    reasoningEffortDescription: "僅對支援推理的模型生效。",
    reasoningEffortDefault: "預設",
    reasoningEffortLow: "低",
    reasoningEffortMedium: "中",
    reasoningEffortHigh: "高",
    noModelsConfigured: "尚未設定模型",
    agentDescription: "自動化任務、搜尋並操作你的瀏覽器",
    askDescription: "詢問網頁相關問題",
    send: "傳送",
    queueMessage: "加入佇列",
    queuedMessages: "佇列中的訊息",
    queuedMessagesDescription: "{count} 則等待中，會在目前回覆結束後傳送。",
    editingMessage: "正在編輯訊息",
    editingMessageDescription: "只有輸入區可操作。按 Esc 或取消即可停止編輯。",
    stop: "停止",
    scrollToTop: "捲動到頂部",
    scrollToBottom: "捲動到底部",
    running: "執行中",
    done: "完成",
    error: "錯誤",
    toolErrors: {
      NO_ACTIVE_WEB_TAB_FOUND:
        "找不到目前活動的網頁分頁。請先開啟或聚焦一般 http(s) 網頁，或使用已知分頁 ID。",
      ACTIVE_TAB_NOT_WEB_PAGE:
        "目前活動分頁不是可讀取的網頁。請先開啟 http(s) 頁面。",
      NO_TAB_IDS_PROVIDED: "未提供分頁 ID。",
      TAB_NOT_FOUND: "找不到指定分頁。",
      NO_GROUPABLE_NORMAL_TABS: "沒有可分組的一般瀏覽器視窗分頁。",
      UNKNOWN_TOOL: "未知工具：{tool}。",
      MISSING_URL: "需要提供 URL。",
      UNKNOWN_NAVIGATION_TYPE: "未知導覽類型：{type}。",
      TAB_HAS_NO_WINDOW: "此分頁不屬於瀏覽器視窗。",
      NO_TEXT_PROVIDED: "未提供要等待的文字。",
      TIMED_OUT_WAITING_FOR_TEXT: "等待指定文字逾時。",
      ELEMENT_NOT_TEXT_INPUT: "目標元素不是輸入框、文字區域或可編輯區域。",
      TARGET_ID_CANNOT_BE_FOCUSED: "此 CDP 目標無法作為瀏覽器分頁聚焦。",
      CLOSE_BY_TARGET_ID_UNSUPPORTED: "此處不支援透過 CDP 目標 ID 關閉頁面。",
      RESIZE_BY_TARGET_ID_UNSUPPORTED:
        "此處不支援透過 CDP 目標 ID 調整視窗大小。",
      ELEMENT_NOT_FOUND: "找不到目標元素。",
      ELEMENT_HAS_NO_CLICKABLE_BOX: "目標元素沒有可點擊區域。",
      MISSING_CODE: "需要提供 JavaScript 程式碼。",
    },
    generatedImage: "生成的圖片",
    capturedTabImage: "分頁截圖",
    mermaidPreview: "Mermaid 圖表預覽",
    openMermaidPreview: "開啟 Mermaid 預覽",
    downloadMermaidSvg: "下載 SVG",
    downloadMermaidPng: "下載 PNG",
    downloadGeneratedImage: "下載圖片",
    downloadCapturedTabImage: "下載截圖",
    copyImage: "複製圖片",
    copyPrompt: "複製提示詞",
    tool: {
      question: { running: "正在提問", done: "已回答問題" },
      openNewTabWithURL: { running: "正在開啟新頁面", done: "已開啟新頁面" },
      findAccessableElementsFromTab: {
        running: "正在查找頁面上的可存取元素",
        done: "已查找頁面上的可存取元素",
        found: "找到 {count} 個可存取元素",
      },
      waitTabLoadFinished: { running: "等待頁面載入", done: "頁面已載入" },
      inputTextByAiID: { running: "正在輸入文字", done: "已輸入文字" },
      clickElementByAiID: { running: "正在點擊元素", done: "已點擊元素" },
      cdpMouseActionByAiID: {
        running: "操作頁面元素",
        done: "已操作頁面元素",
      },
      getTabContent: { running: "正在取得網頁內容", done: "已取得網頁內容" },
      groupTabs: { running: "正在整理分頁", done: "已整理分頁" },
      wait: { running: "等待", done: "已等待" },
      getCurrentTime: { running: "查看時間", done: "已查看時間" },
      getCurrentTab: {
        running: "正在取得目前頁面內容",
        done: "已取得目前頁面內容",
      },
      getAllTabs: {
        running: "正在查看開啟的分頁",
        done: "已查看開啟的分頁",
        found: "找到 {count} 個開啟的分頁",
      },
      goToTab: { running: "正在切換分頁", done: "已切換分頁" },
      openSearchTab: { running: "正在搜尋", done: "已開啟搜尋頁" },
      downloadAllImagesInTab: { running: "正在下載圖片", done: "已下載圖片" },
      downloadTabToMarkdown: {
        running: "正在下載 Markdown",
        done: "已下載 Markdown",
      },
      insertCSSToTab: { running: "正在套用頁面樣式", done: "已套用頁面樣式" },
      removeCSSToTab: { running: "正在移除頁面樣式", done: "已移除頁面樣式" },
      closeTab: { running: "正在關閉分頁", done: "已關閉分頁" },
      reloadTab: { running: "正在重新整理分頁", done: "已重新整理分頁" },
      navigateTab: { running: "正在導覽分頁", done: "已導覽分頁" },
      captureVisibleTab: { running: "正在截取分頁", done: "已截取分頁" },
      waitForText: { running: "等待頁面文字", done: "已找到頁面文字" },
      scrollToBottom: { running: "正在捲動頁面", done: "已捲動頁面" },
      getElementPropertiesByAiID: {
        running: "正在檢查元素",
        done: "已檢查元素",
      },
      listSkills: {
        running: "讀取技能清單",
        done: "已讀取技能清單",
        found: "找到 {count} 個",
      },
      createSkill: { running: "建立技能", done: "已建立技能" },
      readSkill: { running: "讀取技能", done: "已讀取技能" },
      updateSkillFile: { running: "更新技能", done: "已更新技能" },
      patchSkillFile: { running: "修補技能", done: "已修補技能" },
      listMcpServers: { running: "列出 MCP 伺服器", done: "已列出 MCP 伺服器" },
      addMcpServer: { running: "新增 MCP 伺服器", done: "已新增 MCP 伺服器" },
      updateMcpServer: {
        running: "更新 MCP 伺服器",
        done: "已更新 MCP 伺服器",
      },
      testMcpServer: { running: "測試 MCP 伺服器", done: "已測試 MCP 伺服器" },
      deleteMcpServer: {
        running: "刪除 MCP 伺服器",
        done: "已刪除 MCP 伺服器",
      },
      generateImage: { running: "生成圖片", done: "已生成圖片" },
      readFileFromUrl: { running: "正在讀取檔案", done: "已讀取檔案" },
      readUploadedAttachment: { running: "正在讀取附件", done: "已讀取附件" },
      loadBrowserTools: {
        running: "載入瀏覽器工具",
        done: "已載入瀏覽器工具",
        found: "已載入 {count} 個工具",
      },
      cdpClickAt: { running: "點擊座標", done: "已點擊座標" },
      cdpPressKey: { running: "按下按鍵", done: "已按下按鍵" },
      cdpTypeText: { running: "輸入文字", done: "已輸入文字" },
      cdpFill: { running: "填寫欄位", done: "已填寫欄位" },
      cdpFillForm: { running: "填寫表單", done: "已填寫表單" },
      cdpDrag: { running: "拖曳頁面", done: "已拖曳頁面" },
      cdpHandleDialog: { running: "處理對話框", done: "已處理對話框" },
      cdpListPages: { running: "查看頁面", done: "已查看頁面" },
      cdpNewPage: { running: "開啟頁面", done: "已開啟頁面" },
      cdpNavigatePage: { running: "導覽頁面", done: "已導覽頁面" },
      cdpSelectPage: { running: "聚焦頁面", done: "已聚焦頁面" },
      cdpClosePage: { running: "關閉頁面", done: "已關閉頁面" },
      cdpWaitFor: { running: "等待頁面文字", done: "已找到頁面文字" },
      cdpResizePage: { running: "調整頁面大小", done: "已調整頁面大小" },
      cdpEmulate: { running: "套用頁面模擬", done: "已套用頁面模擬" },
      cdpEvaluateScript: { running: "執行頁面腳本", done: "已執行頁面腳本" },
      cdpExecuteArbitraryJavaScript: {
        running: "執行自訂腳本",
        done: "已執行自訂腳本",
      },
      cdpTakeScreenshot: { running: "擷取頁面截圖", done: "已擷取頁面截圖" },
      cdpTakeSnapshot: { running: "取得頁面快照", done: "已取得頁面快照" },
      cdpListConsoleMessages: {
        running: "讀取主控台訊息",
        done: "已讀取主控台訊息",
      },
      cdpGetConsoleMessage: {
        running: "讀取主控台訊息",
        done: "已讀取主控台訊息",
      },
      cdpListNetworkRequests: {
        running: "讀取網路請求",
        done: "已讀取網路請求",
      },
      cdpGetNetworkRequest: { running: "讀取網路請求", done: "已讀取網路請求" },
      cdpPerformanceStartTrace: {
        running: "開始效能追蹤",
        done: "已開始效能追蹤",
      },
      cdpPerformanceStopTrace: {
        running: "停止效能追蹤",
        done: "已停止效能追蹤",
      },
      cdpPerformanceAnalyzeInsight: {
        running: "分析效能洞察",
        done: "已分析效能洞察",
      },
      cdpTakeMemorySnapshot: {
        running: "取得記憶體快照",
        done: "已取得記憶體快照",
      },
      cdpGetMemorySnapshotDetails: {
        running: "讀取記憶體快照",
        done: "已讀取記憶體快照",
      },
      cdpGetNodesByClass: {
        running: "讀取記憶體節點",
        done: "已讀取記憶體節點",
      },
      cdpLoadMemorySnapshot: {
        running: "載入記憶體快照",
        done: "已載入記憶體快照",
      },
      cdpLighthouseAudit: { running: "執行頁面稽核", done: "已執行頁面稽核" },
      cdpScreencastStart: { running: "開始頁面錄製", done: "已開始頁面錄製" },
      cdpScreencastStop: { running: "停止頁面錄製", done: "已停止頁面錄製" },
      readSkillFile: { running: "讀取技能檔案", done: "已讀取技能檔案" },
      listWorkspaceFiles: {
        running: "列出工作區檔案",
        done: "已列出工作區檔案",
      },
      readWorkspaceFile: {
        running: "讀取工作區檔案",
        done: "已讀取工作區檔案",
      },
      writeWorkspaceFile: {
        running: "寫入工作區檔案",
        done: "已寫入工作區檔案",
      },
      patchWorkspaceFile: {
        running: "編輯工作區檔案",
        done: "已編輯工作區檔案",
      },
      deleteWorkspaceFile: {
        running: "刪除工作區檔案",
        done: "已刪除工作區檔案",
      },
      searchWorkspaceFiles: {
        running: "搜尋工作區檔案",
        done: "已搜尋工作區檔案",
      },
      listMemory: { running: "列出記憶", done: "已列出記憶" },
      addMemory: { running: "新增記憶", done: "已新增記憶" },
      updateMemory: { running: "更新記憶", done: "已更新記憶" },
      removeMemory: { running: "移除記憶", done: "已移除記憶" },
      listUserProfile: { running: "列出使用者檔案", done: "已列出使用者檔案" },
      addUserProfileNote: {
        running: "新增使用者檔案記錄",
        done: "已新增使用者檔案記錄",
      },
      updateUserProfileNote: {
        running: "更新使用者檔案記錄",
        done: "已更新使用者檔案記錄",
      },
      removeUserProfileNote: {
        running: "移除使用者檔案記錄",
        done: "已移除使用者檔案記錄",
      },
      searchChatHistory: { running: "搜尋聊天記錄", done: "已搜尋聊天記錄" },
      readChatThread: { running: "讀取聊天串", done: "已讀取聊天串" },
      deleteChatThread: { running: "刪除聊天串", done: "已刪除聊天串" },
      startSubAgent: { running: "啟動子智慧體", done: "已啟動子智慧體" },
      getSubAgentStatus: { running: "檢查子智慧體", done: "已檢查子智慧體" },
      listLocalExecutionBridges: {
        running: "列出本地執行橋",
        done: "已列出本地執行橋",
      },
      startLocalExecutionBridge: {
        running: "啟動本地命令",
        done: "本地命令已完成",
      },
      getLocalExecutionBridgeStatus: {
        running: "檢查本地命令",
        done: "已檢查本地命令",
      },
      cancelLocalExecutionBridge: {
        running: "取消本地命令",
        done: "已取消本地命令",
      },
      addLocalExecutionBridge: { running: "新增執行橋", done: "已新增執行橋" },
      updateLocalExecutionBridge: {
        running: "更新執行橋",
        done: "已更新執行橋",
      },
      testLocalExecutionBridge: { running: "測試執行橋", done: "已測試執行橋" },
      deleteLocalExecutionBridge: {
        running: "刪除執行橋",
        done: "已刪除執行橋",
      },
    },
  },
  options: {
    general: "一般",
    generalDescription: "調整擴充功能的外觀、語言、自動化和上下文行為。",
    agents: "智慧體",
    agentsDescription: "建立擁有獨立角色、行為和工作區的智慧體設定。",
    defaultAgentSummary: "用於瀏覽、搜尋和自動化的瀏覽器操作智慧體。",
    builtinAgentBadge: "內建 · 無工作區/記憶 · 唯讀",
    newAgent: "新增智慧體",
    importAgentZip: "匯入智慧體壓縮包",
    downloadAgentZip: "下載智慧體壓縮包",
    importAgentPackageMissingManifest: "智慧體壓縮包缺少 agent.json。",
    importAgentPackageInvalidManifest: "智慧體壓縮包中的 agent.json 無效。",
    agentName: "智慧體名稱",
    agentDescription: "智慧體描述",
    agentIcon: "智慧體圖示",
    agentIconLabels: {
      bot: "機器人",
      compass: "指南針",
      search: "搜尋",
      help: "說明",
      languages: "語言",
      sparkles: "靈感",
      globe: "地球",
      bookOpen: "書籍",
      wrench: "工具",
      fileText: "文件",
    },
    agentCapabilities: "智慧體能力",
    agentCapabilitiesDescription: "選擇這個智慧體可以使用的工具和上下文。",
    builtinAgentCapabilitiesDescription: "內建智慧體的能力僅供查看，不能修改。",
    agentCapabilityGroups: {
      browser: "瀏覽器",
      builtinTools: "內建工具和上下文",
      skillsAndMcp: "技能和 MCP",
      workspaceAndMemory: "工作區和記憶",
      chatHistory: "聊天記錄",
    },
    agentCapabilityLabels: {
      browserAutomation: "操作瀏覽器",
      browserTools: "使用內建工具",
      subAgents: "啟動子智慧體",
      localExecutionBridges: "使用本地執行橋",
      deferredBrowserTools: "載入進階瀏覽器操作",
      cdpTools: "CDP 工具",
      javascriptExecution: "執行 JavaScript",
      mcpTools: "MCP 工具",
      mcpManagement: "MCP 管理",
      skillTools: "技能工具",
      skillCreation: "建立技能",
      workspaceRead: "讀取工作區",
      workspaceWrite: "寫入工作區",
      memoryRead: "讀取記憶",
      memoryWrite: "寫入記憶",
      chatHistoryRead: "讀取聊天記錄",
      chatHistoryWrite: "寫入聊天記錄",
      imageGeneration: "生成圖片",
      currentTime: "讀取目前時間",
      fileUrlRead: "讀取網址和檔案連結",
    },
    deleteAgent: "刪除智慧體",
    agentWorkspace: "智慧體工作區",
    agentWorkspaceDescription:
      "此智慧體的私有文字檔案。SOUL.md 定義行為，AGENTS.md 定義工作區規則，MEMORY.md 和 USER.md 作為凍結上下文注入，NOTES.md 按需使用。",
    agentWorkspaceNewFile: "新增工作區檔案",
    agentWorkspaceNewFilePlaceholder: "NOTES.md",
    agentWorkspaceEmpty: "尚無工作區檔案。",
    resetDefaultAgents: "重設預設智慧體",
    resetDefaultAgentsDescription:
      "用內建預設智慧體取代目前智慧體。自訂智慧體和工作區會被移除。",
    sync: "同步",
    providers: "提供者",
    mcpServers: "MCP 伺服器",
    localExecutionBridges: "本地執行橋",
    localExecutionBridgesDescription:
      "設定可同步的本地或遠端執行橋。擴充功能把 shell 命令送給橋，由橋在主機上執行。",
    localExecutionBridgesEmpty: "尚未設定本地執行橋。",
    newLocalExecutionBridge: "新增執行橋",
    localExecutionBridgeName: "執行橋名稱",
    localExecutionBridgeDescription: "描述",
    localExecutionBridgeHostName: "原生訊息主機名",
    localExecutionBridgeHostAddress: "執行主機地址",
    localExecutionBridgeHostAddressPlaceholder:
      "留空表示本機，或填寫橋接器識別的地址",
    localExecutionBridgeSecret: "橋接密鑰",
    localExecutionBridgeRegenerateSecret: "重新產生密鑰",
    localExecutionBridgeKey: "Shell 設定 ID",
    localExecutionBridgeDefaultCwd: "預設工作目錄",
    localExecutionBridgeTimeoutMs: "預設逾時（毫秒）",
    localExecutionBridgeTest: "測試橋接器",
    localExecutionBridgeTesting: "測試中...",
    localExecutionBridgeTestSuccess: "橋接器已成功回應。",
    localExecutionBridgeTestError: "橋接器測試失敗",
    deleteLocalExecutionBridge: "刪除執行橋",
    skills: "技能",
    modelProviders: "模型提供者",
    providerDescription:
      "使用自己的金鑰，將 OpenBrowserAgent 連接到你常用的 AI 提供者。",
    mcpServersDescription: "設定助手可使用的遠端 Streamable HTTP MCP 伺服器。",
    newMcpServer: "新增 MCP 伺服器",
    mcpServerName: "伺服器名稱",
    mcpServerDescription: "描述",
    mcpServerUrl: "端點 URL",
    mcpServerUrlPlaceholder: "https://example.com/mcp",
    mcpServerHeaders: "Headers JSON",
    mcpServerEnabled: "已啟用",
    mcpServerEnabledDescription: "將此伺服器設定提供給助手工具。",
    mcpEnableRequiresTest: "請先測試此 MCP 伺服器，然後才能啟用。",
    mcpEnableTooltipTested: "測試已通過，找到 {count} 個工具。",
    mcpAdvancedActions: "MCP 伺服器操作",
    mcpImportJson: "貼上 MCP JSON 設定",
    mcpImportButton: "匯入 MCP JSON",
    mcpImportSuccess: "已匯入 {count} 個 MCP 伺服器。",
    mcpImportNone: "貼上的 JSON 中找不到遠端 HTTP MCP 伺服器。",
    mcpImportError: "MCP 匯入失敗",
    mcpTestServer: "測試伺服器",
    mcpTesting: "正在測試...",
    mcpTestSuccess: "找到 {count} 個工具。",
    mcpTools: "可用工具",
    deleteMcpServer: "刪除 MCP 伺服器",
    resetDefaults: "重設預設值",
    resetBuiltins: "重設內建項目",
    resetDefaultMcpServers: "重設預設 MCP 伺服器",
    resetDefaultMcpServersDescription:
      "用內建預設項目取代目前的 MCP 伺服器。自訂伺服器和測試結果會被移除。",
    resetDefaultSkills: "重設預設技能",
    resetDefaultSkillsDescription:
      "用內建預設項目取代目前技能。自訂技能和修改會被移除。",
    selectModel: "選擇模型",
    selectModelDescription:
      "選擇主要對話模型。智慧體模式建議使用支援工具呼叫的模型。",
    selectImageModel: "選擇圖片模型",
    enableImageGeneration: "啟用圖片生成",
    enableImageGenerationDescription: "向助手開放圖片生成工具。",
    imageGenerationSize: "圖片尺寸",
    providerConnection: "連線資訊",
    providerConnectionHint: "設定提供者名稱、金鑰和端點。",
    providerName: "提供者名稱",
    deleteProvider: "刪除提供者",
    apiKey: "API Key",
    baseUrl: "Base URL",
    models: "模型",
    imageModels: "圖片生成模型",
    modelHint: "先取得模型，再選擇一個新增",
    imageModelHint: "新增支援圖片生成或圖片編輯的模型",
    fetchModels: "取得模型",
    fetchingModels: "正在取得模型",
    searchFetchedModels: "搜尋已取得模型...",
    selectFetchedModel: "選擇已取得模型",
    noFetchedModelsMatch: "沒有符合的已取得模型",
    allFetchedModelsAdded: "已取得模型皆已新增",
    addSelected: "新增所選",
    addCustomModelName: "新增自訂模型名稱",
    addCustomImageModelName: "新增圖片模型名稱",
    addCustom: "新增自訂",
    testModel: "測試",
    testingModel: "測試中",
    testModelSuccess: "模型已成功回應。",
    testModelError: "模型測試失敗",
    configuredModels: "已設定模型",
    noModelsAddedYet: "尚未新增模型。",
    skillsDescription: "使用技能，一鍵向 AI 傳送指令。",
    newSkill: "新增技能",
    importSkillZip: "匯入壓縮包",
    importingSkillZip: "匯入中...",
    importSkillZipError: "技能匯入失敗",
    importSkillPackageMissingEntry: "匯入的壓縮包中找不到 SKILL.md。",
    noSkillsTitle: "尚無技能",
    noSkillsDescription: "建立第一個技能，用一次點擊自動化重複任務。",
    untitledSkill: "未命名操作",
    title: "標題",
    description: "Description",
    instruction: "指令",
    skillFiles: "技能檔案",
    replaceFile: "替換檔案",
    filePath: "檔案路徑",
    downloadFile: "下載檔案",
    downloadSkillZip: "下載壓縮包",
    downloadSkillPackage: "下載 .skill",
    duplicateSkill: "克隆",
    disabled: "已停用",
    addFile: "新增檔案",
    uploadFile: "上傳檔案",
    editFile: "編輯檔案",
    saveFile: "儲存檔案",
    previewFile: "預覽檔案",
    deleteFile: "刪除檔案",
    binaryPreviewUnavailable: "二進位檔案無法預覽。",
    updatedAt: "更新時間",
    skillSize: "技能大小",
    skillCheckEntry: "存在 SKILL.md",
    skillCheckEntryMissing: "缺少 SKILL.md",
    skillCheckName: "中繼資料名稱使用短橫線命名",
    skillCheckNameInvalid: "中繼資料名稱需使用短橫線命名",
    skillCheckDescription: "已填寫描述",
    skillCheckDescriptionMissing: "缺少描述",
    deleteSkill: "刪除技能",
    deleteSkillConfirm: "刪除此技能？",
    availableVariables: "可用變數（點擊複製）：",
    example: "範例",
    languageDescription: "選擇擴充功能介面使用的語言。",
    colorScheme: "配色方案",
    colorSchemeDescription: "選擇按鈕、分頁和高亮使用的柔和主色。",
    appearance: "外觀模式",
    appearanceDescription: "選擇淺色、深色，或跟隨系統設定。",
    greenTheme: "柔和綠色",
    blueTheme: "霧藍色",
    pinkTheme: "淡粉色",
    purpleTheme: "薰衣草紫",
    amberTheme: "暖琥珀",
    systemTheme: "跟隨系統",
    lightTheme: "淺色",
    darkTheme: "深色",
    syncSettings: "同步設定",
    syncSettingsDescription:
      "選擇用於預設同步語言、提供者設定和輕量偏好設定的後端。",
    syncBackend: "同步後端",
    syncBackendDescription: "選擇額外的同步項目。",
    syncBackendBrowserSync: "內建瀏覽器帳號同步",
    syncBackendWebDavSync: "WebDAV 同步",
    syncBackendLocalOnly: "僅本機",
    syncBackendName: "後端名稱",
    syncBackendWebDavUrl: "WebDAV 位址",
    syncBackendUsername: "使用者名稱",
    syncBackendPassword: "密碼或應用程式密碼",
    syncBackendTest: "測試",
    syncBackendTesting: "測試中...",
    syncBackendTestSuccess: "已連線",
    syncBackendTestError: "連線失敗",
    syncBackendCopyConfig: "複製配置碼",
    syncBackendCopiedConfig: "配置碼已複製",
    syncBackendRemove: "移除",
    syncProviders: "同步提供者",
    syncProvidersDescription: "透過目前同步後端同步提供者設定，包括 API Key。",
    syncAgents: "同步智慧體",
    syncAgentsDescription: "透過目前同步後端同步智慧體設定和工作區檔案。",
    syncSkills: "同步技能",
    syncSkillsDescription: "透過目前同步後端同步技能範本。",
    syncMcpServers: "同步 MCP 伺服器",
    syncMcpServersDescription:
      "透過目前同步後端同步 Streamable HTTP MCP 伺服器設定。",
    syncLocalExecutionBridges: "同步本地執行橋",
    syncLocalExecutionBridgesDescription:
      "透過目前同步後端同步執行橋設定、原生主機名、執行主機地址、橋接密鑰、Shell 設定 ID 和預設工作目錄。",
    syncChats: "同步對話",
    syncChatsDescription:
      "透過目前同步後端同步對話記錄。關閉時對話只保存在本機。",
    syncChatAttachments: "同步對話附件",
    syncChatAttachmentsDescription:
      "透過目前支援附件的同步後端同步對話中上傳的檔案。瀏覽器內建同步受配額限制，不支援附件。",
    syncWritePending: "同步寫入排隊中",
    syncWritePendingDetail: "{count} 項正在等待同步",
    syncWriteOperationWrite: "寫入",
    syncWriteOperationRemove: "刪除",
    syncWriteIdle: "同步寫入已完成",
    syncWriteError: "最近一次同步寫入失敗",
    autoScroll: "自動捲動",
    autoScrollDescription: "AI 輸出時自動保持最新內容可見。",
    autoRetry: "自動重試",
    autoRetryDescription: "如果 AI 30 秒沒有新輸出，自動繼續一次。",
    maxToolSteps: "最大工具步數",
    maxToolStepsDescription:
      "設定 AI 最多執行多少次瀏覽器工具；填 0 表示不允許呼叫工具。",
    contextBudget: "上下文預算",
    contextBudgetDescription:
      "限制每次模型請求攜帶的內容，讓長對話保持回應速度，並避免工具輸出擠占任務上下文。",
    contextBudgetEnabled: "啟用上下文預算管理",
    contextBudgetEnabledDescription:
      "只裁剪本次傳送給模型的請求，已儲存的聊天歷史保持完整。",
    contextRequestMaxChars: "請求預算字元數",
    contextTailMinMessages: "保留的最近訊息數",
    contextToolResultMaxChars: "工具結果字元上限",
    debug: "偵錯",
    debugDescription: "用於重設 OpenBrowserAgent 資料的開發工具。",
    debugResetTitle: "清除 OpenBrowserAgent 資料",
    debugResetDescription:
      "刪除 OpenBrowserAgent 寫入的本機儲存、瀏覽器同步儲存、對話、提供者、技能、偏好設定和應用程式同步快取。",
    debugResetConfirmLabel: "輸入 {phrase} 以確認。",
    debugResetConfirmPhrase: "RESET",
    debugResetScope: "儲存位置",
    debugResetScopeAll: "本機 + 雲端",
    debugResetScopeLocal: "僅本機",
    debugResetScopeSync: "僅雲端",
    debugResetTargets: "資料類型",
    debugResetTargetAll: "全部",
    debugResetTargetSettings: "設定",
    debugResetTargetProviders: "提供者",
    debugResetTargetAgents: "智慧體",
    debugResetTargetSkills: "技能",
    debugResetTargetMcpServers: "MCP 伺服器",
    debugResetTargetLocalExecutionBridges: "本地執行橋",
    debugResetTargetChats: "對話",
    debugResetButton: "清除全部應用程式資料",
    debugResetting: "正在清除...",
    debugResetSuccess: "OpenBrowserAgent 資料已清除。",
    debugToolsTitle: "已註冊工具",
    debugToolsDescription: "目前瀏覽器工具註冊表（{count} 個工具）。",
    debugToolAvailable: "可用",
    debugToolUnavailable: "不可用",
    debugToolRequired: "必填",
    debugToolCapabilityEnabled: "由智慧體能力控制：{capability}。",
    debugToolCapabilityDisabled: "需要啟用智慧體能力：{capability}。",
    debugToolImageGenerationDisabled: "需要在提供者中啟用圖片生成。",
    debugToolSkillsDisabled: "需要存在已啟用技能，並開啟所需智慧體能力。",
    debugToolAttachmentsRequired: "需要目前訊息中有上傳附件。",
    debugToolStepsDisabled: "最大工具步數為 0。",
  },
};
